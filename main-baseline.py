...
import torch
import csv
def baseline_save_to_csv(input_data,dataset_names,num_of_residue):
    output_data = {}
    # Dataset names
    #dataset_names = ['diabetes1', 'IBD', 'MS', 'Psoriasis', 'RA', 'SLE']

    # Extract the desired values from the input data and add them to the output_data dictionary
    for key, values in input_data.items():
        output_data[key] = {
            dataset: {'accuracy': values[2], 'standard_deviation': values[1]}
            for dataset in dataset_names
        }

    # Save the output_data dictionary as a CSV file
    with open('output.csv', 'a', newline='') as csvfile:
        fieldnames = ['method-site'] + [f'{dataset}_acc' for dataset in dataset_names] + [f'{dataset}_std' for dataset in dataset_names]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        for key, values in output_data.items():
            row = {'method-site': key+str(num_of_residue)}
            for dataset, metrics in values.items():
                row[f'{dataset}_acc'] = metrics['accuracy']
                row[f'{dataset}_std'] = metrics['standard_deviation']
            writer.writerow(row)
...
#############################################
num_of_selected_residue_loop_set = [25,200,2000]#[2000]#,200,2000]#,200,2000]#1000,2000]#20,30,40,50,100,200,400,500,1000,2000]#,4000]
...
setup_seed(3407)
datasetNameListLoopSet=[['IBD'],['MS'],['Psoriasis'],['RA'],['SLE'],['diabetes1']]

for num_of_selected_residue in num_of_selected_residue_loop_set:
    for datasetNameList in datasetNameListLoopSet:
    #for skip_connection_mode in ["VAE&hdmsk-2enc&batchnorm@p@$20$.50.umapet+100+"]:#,"VAE&hdmsk-4enc-self-fc&batchnorm@p@$20$.50.umapet+100+"]:#"VAE&hdmsk-4enc-self-fc&batchnorm","VAE&hdmsk-2enc","VAE&hdmsk&batchnorm","VAE&batchnorm","unet&batchnorm","VAE&unet&batchnorm"]:#,,"VAE&hdmsk-2enc","VAE&unet&hdmsk","VAE&hdmsk","VAE","unet&hdmsk"]:#,"unet"]:#,"unet","no"]:
        for multi_task_training_policy in ["no~re-val"]:#,"no~pwre-val"]:#,"no~ran","ROnP~re-val"]:#"no~pwre-val","no~re-val","no~ran","ROnPo~pwre-val"]:#"no~ran","no~re-val","no~pwre-val","no~re-ss","no~norm","no~s-gdnm","no~DRO"]:#,"no","ROnP"]:#"ReduceLROnPlateau"]:
            ...
            justToCheckBaseline = True #False
            toFillPoint5 = False #True
            toMask = True
            framework = 'pytorch'
            '''
            if framework=='keras':
            ...
            isMultiDataset = False #True

            datasetNameList = ['RA']#[ 'IBD', 'MS', 'Psoriasis', 'RA',
                               #'SLE','diabetes1']  # "diabetes1","RA","Psoriasis"]#,"RA","Psoriasis"]#,"Psoriasis","IBD"]# ['diabetes1','Psoriasis','SLE']
            model = None
            AE_epoch = 5#800#200#00#00 # *len(datasetNameList)
            NN_epoch = 5#800#200#00#00  # *len(datasetNameList)
            ...
            def data_preprocessing(data_train, isMultiDataset=False, datasetNameList=[''], index=0,
                ...
                if (not (datasetNameList[0] == "GSE66695")) and len(datasetNameList) == 1:#one self collected dataset
                    return data_train, selected_residue_name_list,data_label_df
            ...
            if True or isTrain:
                ...
                if isSelfCollectedDataset and (not isMultiDataset):
                    ...
                    train_data_and_label_df, _ ,label= data_preprocessing(train_data_and_label_df.T,datasetNameList=datasetNameList)
                    train_data, test_data = train_test_split(train_data_and_label_df.T, train_size=0.75, random_state=10)
                    print("label"+"&"*100)
                    print(train_data.loc[:,"Ground Truth"+datasetNameList[0]].T)
                ...
                if (not just_check_data):
                    if (framework == 'keras' or framework == 'pytorch') and toTrainMeiNN == True:
                        ...
                        if justToCheckBaseline:
                            ...
                            baseline_save_to_csv(csd,dataset_names=datasetNameList,num_of_residue=num_of_selected_residue)
